...
export default class MiserEngine {
  ...
  #action(i, j) {
    switch (i) {
      ...
      case 30:
        return this.#fix(j);
      default:
        throw new Error(`In Action method: Verb '${this.#verbString}' was defined in the verbs array, but not implemented yet.`);
    }
  }
  ...
  #drop(j) {
    ...
    if (this.#miserState.pt[j] == -1) {
      ...
      this.#outputText += "----------------------------------------";
      this.#outputText += "** You have encountered a bug that\n";
      this.#outputText += "existed in the original Miser program\n";
      this.#outputText += "from 1981. **\n\n";
      this.#outputText += "This bug is being reproduced here to\n";
      this.#outputText += "preserve the experience a player would\n";
      this.#outputText += "have had playing this game in 1981 on\n";
      this.#outputText += "one of the Commodore PET computers.\n\n";
      this.#outputText += "Except for one thing...the game doesn't\n";
      this.#outputText += "just end unexpectedly here as it did\n";
      this.#outputText += "back then.\n\n";
      this.#outputText += "You get to keep playing as if nothing\n";
      this.#outputText += "bad happened!\n";
      return this.#response(this.#outputText);
    }
  ...
  #north() {
    ...
    if (this.#miserState.cp == 0) this.#outputText += "\nThe door slams shut behind you!\n";
  ...
  */
   ...
   * @returns {MiserResponse}
   */
  #response( outputText, gameOver = false, isError = false) {
    return {
      text: outputText,
      gameOver: gameOver,
      isError: isError,
   ...
   */
  getInventoryItemCount(miserState) {
    let itemCount = 0;
    for (const item of miserState.ol ) {
      if ( item == -1 ) {
        itemCount++;
      }
    }
   ...
   * @param {MiserState} miserState
   */
  setGameState(miserState) {
    this.#miserState = miserState;
  }

  isGameInProgress() {
    if ( this.#miserState.cp > 0 ) {
      return true;
    } else {
      return false;
    }
  ...
  static #rStringShortName = [
    ...
    "East Bedroom",
    "Closet",
    "West Hallway Junction",
    "Center of North-South Hallway",
    "East Hallway Junction",
    "Middle of East Hallway",
    "South End East Hallway",
    "hedge maze",
    "hedge maze",
    "hedge maze",
    "hedge maze",
